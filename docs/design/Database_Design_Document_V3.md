...
#### 2.12 视频生成任务表 (video_generation_jobs) - **V3.0 新增**

记录用户每日学习视频的生成任务状态和元数据。

| 字段名 | 数据类型 | 约束 | 说明 |
| :--- | :--- | :--- | :--- |
| `id` | `INT` | PRIMARY KEY, AUTO_INCREMENT | 任务ID |
| `job_id` | `VARCHAR(255)` | NOT NULL, UNIQUE | 任务唯一标识符 (UUID) |
| `user_id` | `INT` | NOT NULL, FOREIGN KEY | 关联 `users.id` |
| `status` | `VARCHAR(50)` | NOT NULL | 任务状态 ('pending', 'processing', 'completed', 'failed') |
| `video_path` | `VARCHAR(255)` | NULL | 视频文件相对路径 (生成成功后填充) |
| `word_ids` | `JSON` | NOT NULL | 视频包含的单词ID列表 (JSON数组) |
| `created_at` | `TIMESTAMP` | NOT NULL DEFAULT CURRENT_TIMESTAMP | 创建时间 |
| `generation_date` | `DATE` | NOT NULL DEFAULT CURRENT_DATE | **生成日期** (用于判断是否为当日视频) |

**索引**:
*   `UNIQUE KEY (user_id, generation_date)`: 确保每个用户每天只能保留一条有效记录（配合业务逻辑）。
*   `INDEX (status)`: 用于查询处理中的任务。

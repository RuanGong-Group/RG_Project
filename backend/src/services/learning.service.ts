...
export async function getDailyWordsForVideo(userId: number, limit = 7): Promise<{ word: string; meaning: string; id: number }[]> {
  ...
  // 1. 获取今日活跃的词义 (新学 createdAt >= today OR 复习 lastReviewAt >= today)
  ...
  let selectedWords: Array<{ wordId: number; word: string; meaning: string; partOfSpeech?: string }> = [];

  if (todayProgress.length > 0) {
    // 按单词分组（每个单词可能有多个词义）
    const wordMeaningsMap = new Map<number, typeof todayProgress>();
    
    for (const progress of todayProgress) {
      const wordId = progress.meaning.wordId;
      if (!wordMeaningsMap.has(wordId)) {
        wordMeaningsMap.set(wordId, []);
      }
      wordMeaningsMap.get(wordId)!.push(progress);
    }
    
    // 从每个单词中选择一个词义（选择第一个遇到的）
    const uniqueWords = Array.from(wordMeaningsMap.entries()).map(([wordId, progresses]) => {
      const firstProgress = progresses[0];
      return {
        wordId,
        word: firstProgress.meaning.word.word,
        meaning: firstProgress.meaning.definition,
        partOfSpeech: firstProgress.meaning.partOfSpeech
      };
    });
    
    // 如果今日活跃单词数量超过 limit，随机抽取
    if (uniqueWords.length > limit) {
      ...
      selectedWords = uniqueWords.slice(0, limit);
    } else {
      selectedWords = uniqueWords;
    }
  }

  // 2. 如果不足 limit，用待复习的单词填充
  if (selectedWords.length < limit) {
    const remaining = limit - selectedWords.length;
    const selectedWordIds = selectedWords.map(w => w.wordId);
    ...
    // 按单词去重
    const dueWordMeaningsMap = new Map<number, typeof dueProgress[0]>();
    for (const progress of dueProgress) {
      const wordId = progress.meaning.wordId;
      if (!dueWordMeaningsMap.has(wordId) && !selectedWordIds.includes(wordId)) {
        dueWordMeaningsMap.set(wordId, progress);
      }
      
      if (dueWordMeaningsMap.size >= remaining) {
        break;
      }
    }
    
    const dueWords = Array.from(dueWordMeaningsMap.values()).map(progress => ({
      wordId: progress.meaning.wordId,
      word: progress.meaning.word.word,
      meaning: progress.meaning.definition,
      partOfSpeech: progress.meaning.partOfSpeech
    }));
    
    selectedWords = [...selectedWords, ...dueWords];
  }
  ...
  if (selectedWords.length < limit) {
    ...
    // 获取新词义 ID
    const newMeaningIds = await getNewMeanings(userId, user?.currentBookTagId || undefined, remaining * 3); // 多获取一些以备去重
    ...
    if (newMeaningIds.length > 0) {
      ...
      // 按单词去重
      const newWordMeaningsMap = new Map<number, typeof newMeanings[0]>();
      for (const meaning of newMeanings) {
        if (!newWordMeaningsMap.has(meaning.wordId) && !selectedWordIds.includes(meaning.wordId)) {
          newWordMeaningsMap.set(meaning.wordId, meaning);
        }
        
        if (newWordMeaningsMap.size >= remaining) {
          break;
        }
      }
      
      const newWords = Array.from(newWordMeaningsMap.values()).map(meaning => ({
  ...
  // 4. 格式化返回（使用 wordId 作为 id）
  const result = selectedWords.map(w => ({
    id: w.wordId,
    word: w.word,
    meaning: w.meaning,
